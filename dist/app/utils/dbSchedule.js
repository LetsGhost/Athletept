...
async function dbSchedule() {
    try {
        ...
        if (userIds) {
            for (let id in userIds) {
                ...
                if (protocolId) {
                    ...
                    await UserModel.updateOne({ _id: user._id }, { $unset: { protocolExercisePlan: 1 } });
                    await user?.save();
                    protocolCounter++;
                }
                ...
                const currentCheckIn = userCheckIn?.checkIn;
                if (currentCheckIn && userCheckIn) {
                    ...
                    await UserModel.updateOne({ _id: userCheckIn._id }, { $unset: { checkIn: 1 } });
                ...
                const currentExercisePlan = userExercisePlan?.exercisePlan;
                ...
                const userWeightAnalytics = await UserModel.findById(userId).populate("weightAnalytics");
                const currentWeightAnalytics = userWeightAnalytics?.weightAnalytics;
        ...
        logger.info(`Updated ${protocolCounter} protocols, ${checkInCounter} checkIns, ${weekDisplayCounter} weekDisplays, and ${exercisePlanCounter} exercisePlans, and ${weightAnalyticsCounter} weightAnalytics.`, { service: "dbSchedule" });
        return {
            success: true,
        };
    }
