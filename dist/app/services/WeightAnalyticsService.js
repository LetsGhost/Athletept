...
class WeightAnalyticsService {
    async createWeightAnalytics(userId) {
        try {
            ...
            if (userMongoose) {
                ...
                userMongoose.weightAnalytics = createdAnalytics._id;
    ...
    async updateBodyWeightArray(userId, weight) {
        try {
            ...
            const weightAnalytics = await UserModel.findById(userId).populate('weightAnalytics');
    ...
    async deleteWeight(userId, weightIndex) {
        try {
            const weightAnalytics = await UserModel.findById(userId).populate('weightAnalytics');
            if (!weightAnalytics?.weightAnalytics) {
                return {
    ...
    async getWeightAnalytics(userId) {
        try {
            const user = await UserModel.findById(userId);
            if (user) {
