...
class ExercisePlanService {
    async createExercisePlanFromExcel(userId, exerciseFile, warmupFile) {
        try {
    ...
    ;
    async createExercisePlanOnly(userId, exerciseFile) {
        try {
            const { success, exercisePlan } = await createExercisePlan(exerciseFile);
    ...
    async createWarmupSingle(userId, warmupFile) {
        try {
