...
import ExerciseAnalyticsModel from '../models/ExerciseAnalyticsModel.js';
class ExerciseAnalyticsService {
    async createExerciseAnalytics(userId) {
        try {
    ...
    async updateExerciseAnalytics(userId) {
        try {
            const user = await UserModel.findById(userId);
            if (!user) {
                return {
            ...
            let exerciseAnalytics = await UserModel.findById(userId).populate("exerciseAnalytics");
            if (!exerciseAnalytics?.exerciseAnalytics) {
                await this.createExerciseAnalytics(userId).then(async (response) => {
                    if (response.success) {
                        exerciseAnalytics = await UserModel.findById(userId).populate("exerciseAnalytics");
                    }
                });
            }
            // THIS IS THE RIGHT WAY TO DO IT Link: https://mongoosejs.com/docs/typescript/populate.html
            const protocol = await UserModel.findById(userId).populate("protocolExercisePlan");
            if (!protocol) {
                return {
                    success: false,
                    code: 404,
    ...
    async getTopExercises(userId) {
        try {
            const user = await UserModel.findById(userId).populate("exerciseAnalytics");
            if (!user) {
                return {
                    success: false,
                    code: 404,
    ...
    async getExerciseRanking(userId) {
        try {
            const user = await UserModel.findById(userId);
            if (!user) {
                return {
